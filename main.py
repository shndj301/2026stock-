...
st.set_page_config(page_title="📈 주식 데이터 분석 대시보드", layout="wide", initial_sidebar_state="expanded")
...
st.markdown('<p class="main-title">📈 인터랙티브 주식 데이터 분석</p>', unsafe_allow_html=True)
st.markdown('<p class="sub-title">Yahoo Finance 데이터 기반 실시간 분석 대시보드</p>', unsafe_allow_html=True)

# ---------------------- 사이드바 ----------------------
st.sidebar.header("⚙️ 설정")

default_tickers = "AAPL, MSFT, GOOGL, TSLA"
tickers_input = st.sidebar.text_input("티커 입력 (쉼표로 구분)", default_tickers)
...
st.sidebar.markdown("---")
main_ticker = st.sidebar.selectbox("상세 분석 종목", tickers if tickers else ["AAPL"])
...
if df.empty:
    st.error(f"'{main_ticker}' 데이터를 가져올 수 없습니다. 티커를 확인해주세요.")
    st.stop()
...
col2.metric("52주 최고", f"${df['High'].max():,.2f}")
col3.metric("52주 최저", f"${df['Low'].min():,.2f}")
col4.metric("평균 거래량", f"{df['Volume'].mean():,.0f}")
market_cap = info.get("marketCap")
col5.metric("시가총액", f"${market_cap/1e9:,.1f}B" if market_cap else "N/A")

company_name = info.get("longName", main_ticker)
st.caption(f"**{company_name}** ({main_ticker}) · {info.get('sector', '')} / {info.get('industry', '')}")
...
if len(tickers) > 1:
    ...
    for t in tickers:
        ...
        if not d.empty:
            compare_data[t] = (d["Close"] / d["Close"].iloc[0] - 1) * 100
    ...
    st.info("비교하려면 사이드바에서 티커를 2개 이상 입력하세요. (예: AAPL, MSFT, GOOGL)")
...
with st.expander("📋 원본 데이터 보기"):
    ...
    st.download_button("CSV 다운로드", csv, f"{main_ticker}_data.csv", "text/csv")
